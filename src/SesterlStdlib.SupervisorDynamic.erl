...
'make_child_spec'(S102Start, S2973) -> S103Restart = sesterl_internal_prim:decode_option_with_default(S2973, restart, fun() -> 'permanent' end), S104ShutdownOpt = sesterl_internal_prim:decode_option(S2973, shutdown), S105Typ = sesterl_internal_prim:decode_option_with_default(S2973, type, fun() -> 'worker' end), begin S107Shutdown = case {S104ShutdownOpt, S105Typ} of {{'ok', S106Shutdown}, _} -> S106Shutdown; {'error', 'worker'} -> {'timeout', 5000}; {'error', 'supervisor'} -> 'infinity' end, #{restart => S103Restart, shutdown => S107Shutdown, start => S102Start, typ => S105Typ} end.
'make_sup_flags'() -> ?MODULE:'make_sup_flags'(#{}).
'make_sup_flags'(S2974) -> S109Intensity = sesterl_internal_prim:decode_option_with_default(S2974, intensity, fun() -> 1 end), S110Period = sesterl_internal_prim:decode_option_with_default(S2974, period, fun() -> 5 end), #{intensity => S109Intensity, period => S110Period}.
